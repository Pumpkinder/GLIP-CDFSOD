...
sed -i "s/${SHOT}_shot_pseu.json/${SHOT}_shot.json/g" ${DIR}/configs/cdfsod/dataset3.yaml ##有需要则测试文件修改为训练文件用于打伪标签
sed -i "s/${SHOT}_shot.json/00tmpp/g" ${DIR}/configs/cdfsod/dataset3_train.yaml ##有需要则测试文件修改为训练文件用于打伪标签
...
CUDA_VISIBLE_DEVICES=0,1,2,3 python -m torch.distributed.launch --nproc_per_node=4 tools/finetune.py \
      ...
      --evaluate_only_best_on_test --push_both_val_and_test --e_stop ${SHOT_ES} --t_task_id ${ITER} --use_prepared_data \
...
#echo ${MODEL_NAME}
sed -i "s/00tmpp/${SHOT}_shot.json/g" ${DIR}/configs/cdfsod/dataset3_train.yaml ##有需要则测试文件修改为训练文件用于打伪标签
...
python pseuod.py ${DIR}/OUTPUT/eval/${MODEL_NAME:0:-4}/inference/test/bbox.json ${DIR}/DATASET/dataset3/annotations/${SHOT}_shot.json ${DIR}/DATASET/dataset3/annotations/${SHOT}_shot_pseu.json ${the}

sed -i "s/${SHOT}_shot.json/${SHOT}_shot_pseu.json/g" ${DIR}/configs/cdfsod/dataset3.yaml ##有需要则测试文件修改为训练文件用于打伪标签
