...
sed -i "s/1_shot_pseu.json/1_shot.json/g" ${DIR}/configs/cdfsod/dataset2.yaml ##有需要则测试文件修改为训练文件用于打伪标签
sed -i "s/1_shot.json/00tmpp/g" ${DIR}/configs/cdfsod/dataset2_train.yaml ##有需要则测试文件修改为训练文件用于打伪标签


######################## zero shot打标签
# sed -i "s/00tmpp/1_shot.json/g" ${DIR}/configs/cdfsod/dataset2_train.yaml ##有需要则测试文件修改为训练文件用于打伪标签
...
# python pseuod.py ${DIR}/OUTPUT/eval/glip_large_model/inference/test/bbox.json ${DIR}/DATASET/dataset2/annotations/1_shot.json ${DIR}/DATASET/dataset2/annotations/1_shot_pseu.json ${the}

sed -i "s/1_shot.json/1_shot_pseu.json/g" ${DIR}/configs/cdfsod/dataset2.yaml ##有需要则测试文件修改为训练文件用于打伪标签
...
python pseuod.py ${DIR}/OUTPUT/eval/${MODEL_NAME:0:-4}/inference/test/bbox.json ${DIR}/DATASET/dataset2/annotations/1_shot.json ${DIR}/DATASET/dataset2/annotations/1_shot_pseu.json ${the}
...
python tools/test_grounding_net.py --config-file /root/autodl-tmp/GLIP/configs/pretrain/glip_Swin_L.yaml --weight /root/autodl-tmp/GLIP/OUTPUT/data2_10s_1iter_pse0.5/model_0000008.pth \
