...
sed -i "s/1_shot_pseu.json/1_shot.json/g" ${DIR}/configs/cdfsod/dataset1.yaml ##有需要则测试文件修改为训练文件用于打伪标签
sed -i "s/1_shot.json/00tmpp/g" ${DIR}/configs/cdfsod/dataset1_train.yaml ##有需要则测试文件修改为训练文件用于打伪标签
...
#echo ${MODEL_NAME}
sed -i "s/00tmpp/1_shot.json/g" ${DIR}/configs/cdfsod/dataset1_train.yaml ##有需要则测试文件修改为训练文件用于打伪标签
...
python pseuod.py ${DIR}/OUTPUT/eval/${MODEL_NAME:0:-4}/inference/test/bbox.json ${DIR}/DATASET/dataset1/annotations/1_shot.json ${DIR}/DATASET/dataset1/annotations/1_shot_pseu.json ${the}

sed -i "s/1_shot.json/1_shot_pseu.json/g" ${DIR}/configs/cdfsod/dataset1.yaml ##有需要则测试文件修改为训练文件用于打伪标签
