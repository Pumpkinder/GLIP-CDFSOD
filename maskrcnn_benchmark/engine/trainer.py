...
def do_train(
    ...
    for iteration, (images, targets, idxs, positive_map, positive_map_eval, greenlight_map) in enumerate(data_loader, start_iter):
        ...
        #########################################
        #if(iteration == 20):
        #   checkpoint_period = int(checkpoint_period / 3)
        ##########################################
        if val_data_loader and (iteration % checkpoint_period == 0 or iteration == max_iter or iteration == cfg.e_stop_detail):
            ############################
            if(cfg.task_name != None and iteration == cfg.e_stop_detail):
                file_name = str(cfg.task_name)#'/root/autodl-tmp/GLIP/OUTPUT/eval/' + str(cfg.task_name)
                if is_main_process():
        ...
        if iteration == max_iter:
            ...
            break
        if(iteration == int(cfg.e_stop * checkpoint_period) or iteration == cfg.e_stop_detail):
            break
