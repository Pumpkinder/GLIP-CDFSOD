...
import sys
sys.path.append('/root/autodl-tmp/GLIP/')
...
def main():
    ...
    ########################
    parser.add_argument("--e_stop", default=999, type=int)
    parser.add_argument("--e_stop_detail", default=999, type=int) # data3用的参数
    parser.add_argument("--t_task_id", default=1, type=int)

    parser.add_argument("--task_name", default=None, type=str) # data3用的参数
    ...
    cfg.e_stop = args.e_stop
    cfg.e_stop_detail = args.e_stop_detail
    ...
    for task_id, ft_cfg in enumerate(ft_configs, 1):
        for shuffle_seed in shuffle_seeds:
            ...
            ft_output_dir = output_dir + '/ft_task_{}'.format(args.t_task_id)
            ...
            if args.custom_shot_and_epoch_and_general_copy:
                ...
                if args.use_prepared_data:
                    if custom_shot != 0: # 0 means full data training
                        ...
                        print("####################################################################")
                        print("Use ",str(cfg_.DATASETS.TRAIN)," for training!")
                        print("####################################################################")
                        try:
