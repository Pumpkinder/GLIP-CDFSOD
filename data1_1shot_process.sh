...
CUDA_VISIBLE_DEVICES=0,1,2,3 python -m torch.distributed.launch --nproc_per_node=4 tools/finetune.py \
      ...
      --e_stop ${SHOT_ES} --t_task_id ${ITER} --use_prepared_data --seed ${SEED}\
...
#cp --path ${DIR}/OUTPUT/eval/${MODEL_NAME:0:-4}/inference/test/bbox.json ${DIR}/GLIP/predict_result/dataset1_1shot/dataset1_1shot.json

sed -i "s/\"category_id\": 7/\"category_id\": 0/g" ${DIR}/OUTPUT/eval/${MODEL_NAME:0:-4}/inference/test/bbox.json
